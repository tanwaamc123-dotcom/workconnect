...
$navGroups = [
    ...
    'seller' => [
        ...
        [t('group_business', $prefs['language']), [['seller-earnings', t('side_earnings', $prefs['language']), 'topup'], ['seller-analytics', t('side_analytics', $prefs['language']), 'analytics'], ['seller-profile', t('side_profile', $prefs['language']), 'profile'], ['seller-settings', t('side_settings', $prefs['language']), 'settings']]],
    ],
    'admin' => [
        [t('group_management', $prefs['language']), [['admin-users', t('side_users', $prefs['language']), 'users'], ['admin-services', t('side_services', $prefs['language']), 'services'], ['admin-orders', t('side_orders', $prefs['language']), 'orders'], ['admin-messages', t('side_messages', $prefs['language']), 'messages']]],
        [t('group_system', $prefs['language']), [['admin-control', t('side_control_center', $prefs['language']), 'analytics'], ['admin-approvals', t('side_approvals', $prefs['language']), 'moderation'], ['admin-moderation', t('side_moderation', $prefs['language']), 'moderation'], ['admin-categories', t('side_categories', $prefs['language']), 'categories'], ['admin-coupons', t('side_coupons', $prefs['language']), 'coupon'], ['admin-logs', t('side_logs', $prefs['language']), 'logs'], ['admin-broadcast', t('side_broadcast', $prefs['language']), 'broadcast'], ['admin-export', t('side_export', $prefs['language']), 'export'], ['admin-reports', t('side_reports', $prefs['language']), 'reports'], ['admin-finance', t('side_finance', $prefs['language']), 'analytics'], ['admin-settings', t('side_system_settings', $prefs['language']), 'settings']]],
    ],
];
?>
<header class="workspace-topbar">
    <button class="workspace-menu" type="button" aria-label="Open workspace navigation"><span></span><span></span></button>
    ...
    <div class="workspace-top-actions">
        ...
        <?php if ($user['role'] === 'customer'): ?><a class="top-wallet" href="?page=topup" aria-label="<?= e(t('nav_topup', $prefs['language'])) ?>"><span><?= icon_svg('topup') ?></span><strong><?= money($walletBalance) ?></strong></a><?php endif; ?>
        <a class="top-profile" href="?page=<?= $user['role'] === 'seller' ? 'seller-profile' : ($user['role'] === 'admin' ? 'admin-settings' : 'profile') ?>"><span class="<?= $user['avatar'] ? 'has-image' : '' ?>" <?= $user['avatar'] ? 'style="background-image:url('.e(upload_url($user['avatar'])).')"' : '' ?>><?= $user['avatar'] ? '' : e(initials($user['name'])) ?></span><div><strong><?= e($user['name']) ?></strong><small><?= (int)$user['is_demo'] === 1 ? 'Demo · ' : '' ?><?= e($user['role_label']) ?></small></div></a>
    </div>
</header>
<aside class="workspace-sidebar">
    ...
    <div class="sidebar-footer-links"><?php if (demo_management_allowed($user) && demo_is_installed()): ?><a class="sidebar-demo-link" href="?page=admin-settings#demo"><i></i><span><strong>Demo data active</strong><small>Manage demo</small></span></a><?php endif; ?><a class="workspace-home-link" href="?page=home">← WorkConnect Home</a></div>
...
<main class="workspace-main">
    <?php foreach (pull_flashes() as $flash): ?><div class="flash <?= e($flash['type']) ?>" role="status"><span><?= $flash['type'] === 'success' ? '✓' : ($flash['type'] === 'error' ? '!' : 'i') ?></span><p><?= e($flash['message']) ?></p><button type="button" aria-label="Dismiss">×</button></div><?php endforeach; ?>
